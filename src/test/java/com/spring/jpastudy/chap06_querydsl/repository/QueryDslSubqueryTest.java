...
class QueryDslSubqueryTest {
    ...
    @Test
    @DisplayName("그룹별 가장 최근에 발매된 앨범 정보 조회")
    void subqueryTest2() {
        /*
    ...
    @Test
    @DisplayName("아이브의 평균 나이보다 나이가 많은 여자 아이돌의 이름,나이 조회")
    void subqueryTest() {
        // 서브쿼리: 아이브 그룹의 평균 나이를 계산
        JPQLQuery<Double> subQuery = JPAExpressions
                .select(idol.age.avg())
                .from(idol)
                .where(group.groupName.eq("아이브"));

        // 메인쿼리: 아이브의 평균 나이보다 나이가 많은 여자 아이돌 조회
        List<Idol> result = factory
                .selectFrom(idol)
        ...
        assertFalse(result.isEmpty());
        for (Idol i : result) {
            System.out.println("\n아이브의 평균 나이보다 나이가 많은 여자 아이돌: "
                    + "\n이름: " + i.getIdolName() + "나이: " + i.getAge());
        }
    }
    ...
    void noAlbumIn2023() {
        ...
        // 서브쿼리: 2023년도에 발매된 앨범이 없는 그룹을 조회
        JPQLQuery<Long> subQuery = JPAExpressions
                .select(album.group.id)
                .from(album)
                .where(album.releaseYear.eq(albumYear));

        // 메인쿼리: 서브쿼리 결과가 존재하지 않는 그룹 조회
        List<Group> result = factory
                .selectFrom(group)
        ...
        for (Group group : result) {
            System.out.println("\n2023년도에 발매된 앨범이 없는 그룹: " + group.getGroupName());
        }
    }
